...
class Keyword(Cipher):
    ...
    def __init__(self, keyword=None):
        ...
        if not keyword:
            self.input_keyword = list(input(
                'Enter a keyword of unique letters \n> '
                ).upper())
        else:
            self.input_keyword = keyword.upper()
        # Creating 2 lists of alphabet.
        # One of regular alphabet. Other to modify with keyword
        self.alphabet = list(string.ascii_uppercase)
        self.alphabet_removed = copy.copy(self.alphabet)

    # Creates the alphabet_removed list by removing
    # letters from list and then adding keyword at beginning
    ...
    def encrypt(self, text):
        '''Encrypts message by matching index of
           letter to encrypted alphabet index.'''
        final_output = []
    ...
    def decrypt(self, text):
        '''Decrypts message by matching index of
           letter to encrypted alphabet index.'''
        final_output = []
        keyword_alphabet = self.alphabet_remove_keyword()
